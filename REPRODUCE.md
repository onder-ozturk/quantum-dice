...
For the **two-backend run with M3 readout-error mitigation** (a single run each), install
`mthree` and run:
```bash
...
`paper/figures/ibm_mitigation_results.json` (used for the Lemma's raw-vs-conditioned TVD ratio).

For the **five-run, two-backend characterisation reported in the paper** (Section IX-D/IX-E,
Table III, Fig. 4):
```bash
PYTHONPATH=quantum-service python paper/figures/run_ibm_repeated.py
```
This calibrates M3 once per backend, then performs five independent 4000-shot runs on each,
writing `paper/figures/ibm_repeated_results.json` (per-run TVD plus mean±s.d.); re-run step 3
to regenerate `ibm-repeated.pdf`. The reported runs gave raw 1–6 TVD = 0.015±0.006
(`ibm_marrakesh`) and 0.032±0.006 (`ibm_kingston`), mitigated 0.017±0.008 and 0.014±0.004.
Your numbers will differ by device, calibration, and run.
